...
if(isset($_POST['add'])){
	...
	$units=$_POST['units'];
	$laborers=intval($_POST['laborers']);
	...
	if($copy=="1"){
		...
		for($i=0;$i<sizeof($fields);$i++){
			$f=$fields[$i];
			if($f==$field){
				$query="INSERT INTO log (activity_id, user_id, field_id, plots, log_date, log_value_number, log_value_units, log_number_of_laborers, log_cost, log_comments, log_picture) VALUES ($id,$user,$field,'$plots','$date',$value,'$units',$laborers,$cost,'$comments','$picture')";
				$result = mysqli_query($dbh,$query);
			} else {
				$copied_comments=$comments." (copied)";
				$copied_plots=getEquivalentPlots($field,$plots,$f,$dbh);
				$query="INSERT INTO log (activity_id, user_id, field_id, plots, log_date, log_value_number, log_value_units, log_number_of_laborers, log_cost, log_comments, log_picture) VALUES ($id,$user,$f,'$copied_plots','$date',$value,'$units',$laborers,$cost,'$copied_comments','$picture')";
				$result = mysqli_query($dbh,$query);
			}
		}
	} else {
	...
	$query="SELECT activity_name, activity_category, activity_measurement_units FROM activity WHERE activity_id=$id";
	$result = mysqli_query($dbh,$query);
	$row = mysqli_fetch_array($result,MYSQL_NUM);
	$activity_name = $row[0];
	$activity_category = $row[1];
	$activity_units = $row[2];
...
<?php
$plots=getRemainingPlots($dbh,$field,array(),($id*-1),"la");
if($plots!=""){
...
?>
<br>
<b>Activity:</b> <?php echo($activity_name." (".$activity_category.")"); ?><br><br>
<b>Date:</b>
...
<b>Units:</b> <input class="w3-input w3-border-green w3-text-green" name="units" id="units" type="text" value="<?php echo($activity_units); ?>">
...
<b>Number of laborers:</b> <input class="w3-input w3-border-green w3-text-green" name="laborers" type="text" value="" onkeypress="return isNumberKey(event)">
<b>Cost:</b> <input class="w3-input w3-border-green w3-text-green" name="cost" type="text" value="" onkeypress="return isNumberKey(event)"><br>
